...
export default function Home() {
  ...
  const [raeComplete, setRaeComplete] = useState(false);

  const handleRaeComplete = useCallback(() => {
    setRaeComplete(true);
  ...
  // ── Navbar: muncul langsung saat raeComplete ──
  const [navReady, setNavReady] = useState(false);
  useEffect(() => {
    if (raeComplete) setNavReady(true);
  }, [raeComplete]);
  ...
  return (
    <div className="text-white overflow-x-hidden font-outfit" style={{ background: '#000000' }}>
      ...
      {/* ══ RAE — fixed overlay, scroll-jacked, hilang saat onComplete ══ */}
      {!raeComplete && (
        <RaePeekSection onComplete={handleRaeComplete} />
      )}

      {/* ── Scroll hint (hanya saat konten aktif) ── */}
      {raeComplete && (
        <motion.div
        ...
        >
          <div
            className="w-px h-8 animate-[va_3s_ease-in-out_infinite]"
            style={{ background: `linear-gradient(to bottom, ${PO(0.35)}, transparent)` }}
          />
          <span
            className="text-[10px] tracking-[5px] uppercase italic"
      ...
      {/* ══ MAIN CONTENT — hanya muncul setelah raeComplete ══ */}
      {raeComplete && (
        <div ref={contentRef} style={{ position: 'relative', zIndex: 20, background: '#080604' }}>
            ...
            <section id="skills" className="scroll-mt-24">
              ...
              <div className="grid md:grid-cols-3 gap-6">
                {[
                  ...
                  <GlareHover
                    key={title} width="100%" height="auto" background="transparent"
